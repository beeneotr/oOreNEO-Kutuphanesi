...
kbr_NeoPixel::kbr_NeoPixel(uint16_t n, int16_t p, neoPixelType t)
...
void kbr_NeoPixel::updateLength(uint16_t n) {
  free(pixels); 
...
void kbr_NeoPixel::updateType(neoPixelType t) {
...
#if defined(ARDUINO_ARCH_RP2040)
void kbr_NeoPixel::rp2040Init(uint8_t pin, bool is800KHz)
{
...
void  kbr_NeoPixel::rp2040Show(uint8_t pin, uint8_t *pixels, uint32_t numBytes, bool is800KHz)
{
  if (this->init)
  {
...
#define CYCLES_400 156 // ~2.50 uS
  ...
  if (pwm != NULL) {
#if defined(ARDUINO_NRF52_KIBRIS) // use thread-safe malloc
...
#if defined(ARDUINO_NRF52_KIBRIS) || defined(ARDUINO_ARCH_NRF52840)
      yield();
#endif
    }
...
#if defined(ARDUINO_NRF52_KIBRIS) // use thread-safe free
...
// Fall back to DWT
#if defined(ARDUINO_NRF52_KIBRIS)
...
*/
void kbr_NeoPixel::setPin(int16_t p) {
  if (begun && (pin >= 0))
...
*/
void kbr_NeoPixel::setPixelColor(uint16_t n, uint8_t r, uint8_t g,
                                      uint8_t b) {
...
*/
void kbr_NeoPixel::setPixelColor(uint16_t n, uint32_t c) {
  if (n < numLEDs) {
...
*/
void kbr_NeoPixel::fill(uint32_t c, uint16_t first, uint16_t count) {
  uint16_t i, end;
...
*/
uint32_t kbr_NeoPixel::ColorHSV(uint16_t hue, uint8_t sat, uint8_t val) {
...
*/
uint32_t kbr_NeoPixel::getPixelColor(uint16_t n) const {
  if (n >= numLEDs)
...
*/
void kbr_NeoPixel::setBrightness(uint8_t b) {
...
*/
uint8_t kbr_NeoPixel::getBrightness(void) const { return brightness - 1; }

/*!
  @brief   Fill the whole NeoPixel strip with 0 / black / off.
*/
void kbr_NeoPixel::clear(void) { memset(pixels, 0, numBytes); }

// A 32-bit variant of gamma8() that applies the same function
// to all components of a packed RGB or WRGB value.
uint32_t kbr_NeoPixel::gamma32(uint32_t x) {
  uint8_t *y = (uint8_t *)&x;
...
*/
void kbr_NeoPixel::rainbow(uint16_t first_hue, int8_t reps,
